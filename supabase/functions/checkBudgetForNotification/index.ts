...
async function sendEmailViaBrevo(toEmail, subject, content) {
...
async function checkBudgetForNotification() {
  const { data: users, error } = await supabase.from('Users').select('id, email, budget, lastNotificationDate, user_id');
  if (error) {
  ...
  for (const user of users){
    try {
      ...
      if (user.lastNotificationDate) {
        ...
        if (lastNotification.getFullYear() === currentYear && lastNotification.getMonth() + 1 === currentMonth) {
          console.log(`Notification already sent for user ${user.id} this month.`);
          continue;
        }
      }
      const { data: expenses, error: expenseError } = await supabase.from('Expenses').select('expenseAmount, expenseDate').eq('user_id', user.user_id);
      if (expenseError) {
      ...
      if (monthlyExpense >= user.budget * 0.8) {
        ...
        const { error: updateError } = await supabase.from('Users').update({
          lastNotificationDate: currentDate.toISOString().split('T')[0]
        }).eq('id', user.id);
        if (updateError) {
